...
async def _get_browser() -> uc.Browser:
    ...
    async with _browser_lock:
        if _browser is None or _browser.stopped:
            ...
            if proxy:
                # nodriver supports per-context proxy via create_context
...
async def drag(source_selector: str, target_selector: str, timeout: int = 10) -> str:
    ...
    try:
        ...
        src_pos = src.backend_node_id  # we need positions
        # Use mouse_drag with element positions
...
async def take_snapshot() -> str:
    """Get the current page's DOM snapshot (HTML source)."""
    tab = await _active_tab()
...
# ---------------------------------------------------------------------------
# Cloudflare / anti-bot bypass helpers
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Additional input tools (matching chrome-devtools-mcp)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Enhanced emulation (network throttling, CPU, geolocation, color scheme)
# ---------------------------------------------------------------------------
...
async def emulate(
    ...
    if user_agent:
        import nodriver.cdp.network as cdp_net
        await tab.send(cdp_net.set_user_agent_override(user_agent=user_agent))
        results.append(f"user_agent set")
...
# ---------------------------------------------------------------------------
# Performance tools (CDP-based trace)
# ---------------------------------------------------------------------------
...
async def performance_stop_trace(file_path: str = "") -> str:
    ...
    tab = await _active_tab()
    import nodriver.cdp.tracing as cdp_tracing
    import nodriver.cdp.io as cdp_io

    if not _tracing_active:
        return json.dumps({"error": "No trace is running."})

    # Collect trace data via events
    trace_chunks = []
...
# ---------------------------------------------------------------------------
# Enhanced navigation (back/forward/reload)
# ---------------------------------------------------------------------------
